...
class Script(TestSuite):
        ...
        for line in lines:
            try:
                ...
                if not some_parsing_failed:
                    _print(
                        f"Some lines could not be parsed in script {self.name}. "
                        "(That's probably not really critical)"
                    )
    ...
    def containsregex(self, regex: str) -> bool:
        ...
        For instance, "app setting" is contained in "yunohost app setting $app..."
        """
        return any(re.search(regex, line) for line in [" ".join(line) for line in self.lines])
    ...
    def error_handling(self) -> TestResult:
        ...
        if (
            ...
            yield ReportError(
                "ynh_abort_if_errors or set -eu is now handled by YunoHost core in packaging v2, "
                "you should not have to add it to your script !"
            )
        ...
        if self.name in ["remove", "_common.sh"]:
            if present:
                yield ReportError(
                    ...
                    "scripts: If a single instruction fails, it will stop the script and is "
    ...
    def obsolete_helpers(self) -> TestResult:
        if self.contains("yunohost app setting"):
            yield ReportCritical(
                "Do not use 'yunohost app setting' directly. Please use "
                "'ynh_app_setting_(set,get,delete)' instead."
            )
        if self.contains("ynh_detect_arch"):
            yield ReportWarning(
                "(Requires yunohost 4.3) Using ynh_detect_arch is deprecated, since Yunohost 4.3, "
                "an $YNH_ARCH variable is directly available in the global context. Its value "
                "directly corresponds to `dpkg --print-architecture` which returns a value among: "
                "amd64, i386, armhf, arm64 and armel (though armel is probably not used at all?)"
            )
    ...
    def deprecated_YNH_APP_ARG(self) -> TestResult:  # noqa: N802
        ...
        if subprocess.call(cmd, shell=True) == 0:
            yield ReportWarning(
                ...
                "saved as settings and are directly available as bash variable $foobar "
                "(instead of $YNH_APP_ARG_FOOBAR)"
            )
    ...
    def deprecated_replace_string(self) -> TestResult:
        ...
        if count2 > 0 or count1 >= 5:
            yield ReportInfo(
                "Please consider using 'ynh_add_config' to handle config files instead of "
                "gazillions of manual cp + 'ynh_replace_string' + chmod"
            )
    ...
    def bad_if_syntax(self) -> TestResult:
        ...
        if res:
            yield ReportWarning(
                "Syntaxes like « if [ $(cmd) ] » is pretty much a nonsense in bash and probably "
    ...
    def ynh_add_fpm_config_deprecated_package_option(self) -> TestResult:
        if self.containsregex(r"ynh_add_fpm_config .*package=.*"):
            yield ReportError(
                "(Requires Yunohost 4.3) Option --package for ynh_add_fpm_config is deprecated: "
                "please use 'ynh_install_app_dependencies' with **all** your apt dependencies "
                "instead (no need to define a special 'extra_php_dependencies'). YunoHost will "
                "automatically install any phpX.Y-fpm / phpX.Y-common if needed."
            )
    ...
    def set_is_public_setting(self) -> TestResult:
        if self.containsregex(r"ynh_app_setting_set .*is_public.*"):
            if self.name == "upgrade":
                yield ReportError(
                    "permission system: it should not be needed to save is_public with "
                    "ynh_app_setting_set... this setting should only be used during installation "
                    "to initialize the permission. The admin is likely to manually tweak the "
                    "permission using YunoHost's interface later."
                )
            else:
                yield ReportWarning(
                    "permission system: it should not be needed to save is_public with "
    ...
    def default_php_version_in_common(self) -> TestResult:
        if self.contains("YNH_DEFAULT_PHP_VERSION"):
            yield ReportWarning(
                "Do not use YNH_DEFAULT_PHP_VERSION in _common.sh... _common.sh is usually "
                "sourced *before* the helpers, which define the version of YNH_DEFAULT_PHP_VERSION "
                "(hence it gets replaced with empty string). Instead, please explicitly state the "
                "PHP version in the package, e.g. dependencies='php8.2-cli php8.2-imagemagick'"
            )
    ...
    def get_is_public_setting(self) -> TestResult:
        if self.contains("is_public=") or self.contains("$is_public"):
            yield ReportWarning(
                ...
                "is_public should only be used during installation to initialize the permission. "
                "The admin is likely to manually tweak the permission using YunoHost's "
                "interface later."
            )

    @test(only=["upgrade"])
    def temporarily_enable_visitors_during_upgrade(self) -> TestResult:
        if self.containsregex("ynh_permission_update.*add.*visitors") and self.containsregex(
            "ynh_permission_update.*remove.*visitors"
        ):
            yield ReportWarning(
                "permission system: since Yunohost 4.3, there should be no need to temporarily "
                "add 'visitors' to the main permission. ynh_local_curl will temporarily enable "
                "visitors access if needed"
            )

    @test()
    def set_legacy_permissions(self) -> TestResult:
        if self.containsregex(r"ynh_app_setting_set .*protected_uris") or self.containsregex(
            r"ynh_app_setting_set .*skipped_uris"
        ):
            yield ReportError(
                "permission system: it looks like the app is still using super-legacy "
                "(un)protected/skipped_uris settings. This is now completely deprecated. "
                "Please check https://yunohost.org/packaging_apps_permissions for a documentation "
                "on how to migrate the app to the new permission system."
            )

        elif self.containsregex(r"ynh_app_setting_set .*protected_") or self.containsregex(
            r"ynh_app_setting_set .*skipped_"
        ):
            yield ReportWarning(
                "permission system: it looks like the app is still using the legacy permission "
                "system (unprotected/protected/skipped uris/regexes setting). "
                "Please check https://yunohost.org/packaging_apps_permissions for a documentation "
                "on how to migrate the app to the new permission system."
            )
    ...
    def normalize_url_path(self) -> TestResult:
        if self.contains("ynh_normalize_url_path"):
            yield ReportWarning(
                "You probably don't need to call 'ynh_normalize_url_path'... this is only "
                "relevant for upgrades from super-old versions (like 3 years ago or so...)"
            )
    ...
    def raw_systemctl_start(self) -> TestResult:
        if self.containsregex(r"systemctl start \"?[^. ]+(\.service)?\"?\s"):
            yield ReportWarning(
                "Please do not use 'systemctl start' to start services. Instead, you should use "
                "'ynh_systemd_action' which will display the service log in case it fails to "
                "start. You can also use '--line_match' to wait until some specific word appear "
                "in the log, signaling the service indeed fully started."
            )
    ...
    def bad_line_match(self) -> TestResult:
        ...
        if self.containsregex(r"--line_match=Started$") or self.containsregex(
            ...
            yield ReportWarning(
                'Using --line_match="Started" or "Stopped" in ynh_systemd_action is counter '
                "productive because it will match the systemd message and not the actual app "
                "message... Please check the log of the service to find an actual, relevant "
                "message to match, or remove the --line_match option entirely"
            )
    ...
    def quiet_systemctl_enable(self) -> TestResult:
        ...
        if any("-q" not in cmd for cmd in systemctl_enable):
            message = "Please add --quiet to systemctl enable/disable commands to avoid "
            "unnecessary warnings when the script runs"
            yield ReportWarning(message)

    @test()
    def quiet_wget(self) -> TestResult:

        wget_cmds = [
            line for line in [" ".join(line) for line in self.lines] if re.search(r"^wget ", line)
        ]

        if any(" -q " not in cmd and "--quiet" not in cmd and "2>" not in cmd for cmd in wget_cmds):
            message = "Please redirect wget's stderr to stdout with 2>&1 to avoid unecessary "
            "warnings when the script runs (yes, wget is annoying and displays a warning even "
            "when things are going okay >_>...)"
            yield ReportWarning(message)
    ...
    def firewall_consistency(self) -> TestResult:
        if self.contains("yunohost firewall allow") and not self.contains("--needs_exposed_ports"):
            yield ReportInfo(
                "You used 'yunohost firewall allow' to expose a port on the outside but did not "
                "use 'yunohost service add' with '--needs_exposed_ports'... If you are ABSOLUTELY "
                "SURE that the service needs to be exposed on THE OUTSIDE, then add "
                "'--needs_exposed_ports' to 'yunohost service add' with the relevant port number. "
                "Otherwise, opening the port leads to a significant security risk and you should "
                "keep the damn port closed !"
            )

        if self.contains("Configuring firewall") and not self.contains("yunohost firewall allow"):
            yield ReportWarning(
                "Some message is talking about 'Configuring firewall' but there's no mention of "
                "'yunohost firewall allow'... If you're only finding an available port for "
                "*internal reverse proxy*, this has nothing to do with 'Configuring the firewall', "
                "so the message should be changed to avoid confusion... "
            )
    ...
    def exit_ynhdie(self) -> TestResult:
        ...
        if self.contains(r"\bexit\b"):
            yield ReportError("'exit' command shouldn't be used. Please use 'ynh_die' instead.")
    ...
    def ssowatconf_or_nginx_reload(self) -> TestResult:
        ...
        # (the "bad" practice being using this at the very end of the script, but some apps
        # legitimately need this in the middle of the script)
        oldlines = list(self.lines)
        self.lines = self.lines[-10:]
        if self.contains("yunohost app ssowatconf"):
            yield ReportWarning(
                "You probably don't need to run 'yunohost app ssowatconf' in the app self. "
                "It's supposed to be ran automatically after the script."
            )

        if self.name not in ["change_url", "restore"]:  # noqa: SIM102
            if self.contains("ynh_systemd_action --service_name=nginx --action=reload"):
                yield ReportWarning(
                    "You should not need to reload nginx at the end of the script... it's already "
                    "taken care of by ynh_add_nginx_config"
                )
    ...
    def sed(self) -> TestResult:
        if self.containsregex(r"sed\s+(-i|--in-place)\s+(-r\s+)?s") or self.containsregex(
            r"sed\s+s\S*\s+(-i|--in-place)"
        ):
            yield ReportInfo(
                "You should avoid using 'sed -i' for substitutions, please use "
                "'ynh_replace_string' or 'ynh_add_config' instead"
            )
    ...
    def sudo(self) -> TestResult:
        if self.containsregex(
            ...
            yield ReportWarning(
                "You should not need to use 'sudo', the script is being run as root. (If you need "
                "to run a command using a specific user, use 'ynh_exec_as' (or 'sudo -u'))"
            )
    ...
    def chownroot(self) -> TestResult:
        ...
        if self.containsregex(r"^\s*chown.* root:?[^$]* .*install_dir") and not self.contains(
            'chown root:root "$install_dir"'
        ):
            # (Mywebapp has a special case because of SFTP é_è)
            yield ReportWarning(
                "Using 'chown root $install_dir' is usually symptomatic of misconfigured and "
                "wide-open 'other' permissions... Usually ynh_setup_source should now set sane "
                "default permissions on $install_dir (if the app requires Yunohost >= 4.2)... "
                "Otherwise, consider using 'chown $app', 'chown nobody' or 'chmod' to limit "
                "access to $install_dir..."
            )
    ...
    def chmod777(self) -> TestResult:
        if self.containsregex(r"chmod .*777") or self.containsregex(r"chmod .*o\+w"):
            yield ReportWarning(
                "DO NOT use chmod 777 or chmod o+w that gives write permission to every users "
                "on the system!!! If you have permission issues, just make sure that the owner "
                "and/or group owner is right..."
            )
    ...
    def random(self) -> TestResult:
        if self.contains("dd if=/dev/urandom") or self.contains("openssl rand"):
            yield ReportError(
                "Instead of 'dd if=/dev/urandom' or 'openssl rand', you should use "
                "'ynh_string_random'"
            )
    ...
    def progression_in_backup(self) -> TestResult:
        if self.contains("ynh_script_progression"):
            yield ReportWarning(
                ...
                "Consider using a simple message like this instead: "
    ...
    def progression_time(self) -> TestResult:
        ...
        if self.containsregex(r"ynh_script_progression.*--time"):
            yield ReportInfo(
                "Using 'ynh_script_progression --time' should only be for calibrating the weight "
                "(c.f. '--weight'). It's not meant to be kept for production versions."
            )
    ...
    def progression_meaningful_weights(self) -> TestResult:
        ...
        if len(weights) > 3 and statistics.stdev(weights) > 50:
            yield ReportWarning(
                "To have a meaningful progress bar, try to keep the weights in the same range of "
                "values, for example [1,10], or [10,100]... otherwise, if you have super-huge "
                "weight differences, the progress bar rendering will be completely dominated by "
                "one or two steps... If these steps are really long, just try to indicated in "
                "the message that this will take a while."
            )

    @test(only=["install", "_common.sh"])
    def php_deps(self) -> TestResult:
        if self.containsregex("dependencies.*php-"):  # noqa: SIM102
            # (Stupid hack because some apps like roundcube depend on php-pear or php-php-gettext
            # and there's no phpx.y-pear phpx.y-php-gettext>_>...
            if not self.contains("php-pear") or not self.contains("php-php-gettext"):
                yield ReportWarning(
                    ...
                    "exact version you want like 'php7.0-foobar'. Otherwise, the *wrong* version "
                    "of the dependency may be installed if sury is also installed. Note that for "
                    "Stretch/Buster/Bullseye/... transition, YunoHost will automatically patch "
                    "your file so there's no need to care about that."
                )
    ...
    def systemd_during_backup(self) -> TestResult:
        if self.containsregex("^ynh_systemd_action"):
            yield ReportWarning(
                "Unless you really have a good reason to do so, starting/stopping services "
                "during backup has no benefit and leads to unecessary service interruptions "
                "when creating backups... As a 'reminder': apart from possibly database dumps "
                "(which usually do not require the service to be stopped) or other super-specific "
                "action, running the backup script is only a *declaration* of what needs to be "
                "backed up. The real copy and archive creation happens *after* the backup "
                "script is ran."
            )
    ...
    def helpers_sourcing_after_official(self) -> TestResult:
        ...
        if helpers_after_official:
            ...
            yield ReportWarning(
                "Please avoid sourcing additional helpers after the official helpers "
                f"(in this case file {helpers_after_official_str})"
            )
    ...
    def no_progress_in_common(self) -> TestResult:
        if self.contains("ynh_script_progression"):
            yield ReportWarning(
                "You should not use `ynh_script_progression` in _common.sh because it will "
                "produce warnings when trying to install the application."
            )
    ...
    def no_log_remove(self) -> TestResult:
        if self.containsregex(r"(ynh_secure_remove|ynh_safe_rm|rm).*(\/var\/log\/)"):
            yield ReportWarning(
                "Do not delete logs on app removal, else they will be erased if the "
                "app upgrade fails. This is handled by the core."
            )
